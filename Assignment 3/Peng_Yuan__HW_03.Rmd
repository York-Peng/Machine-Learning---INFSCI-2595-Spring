...
```{r, solution_01a}
### your code here
model_pred_df %>% group_by(obs_class) %>% count()
```
...
**The calculated result is greater than 0.4, so we consider it a balanced data set**
...
model_pred_df <- model_pred_df %>%
                                mutate(y = ifelse(model_pred_df$obs_class == "event", 
                                                    1, 
                                                    0))
...
What do you think?  
**According to the prediction line, in the interval from x = -1 to 0.5, we can observe that the data is decreasing and reaches its lowest value at x = 0.5. As x increases beyond 0.5, the amount of data that can be observed increases again. This result is the same as the observed-predicted relationship presented by the scatter plot.**
...
model_class_0.5 <- model_pred_df %>%
                                mutate(pred_class = ifelse(model_pred_df$pred_prob > 0.5, 
                                                    "event", 
                                                    "non_event"))
...
```{r, solution_02b}
### your code here
accuracy <- model_class_0.5 %>% count(obs_class == pred_class)
...
**There are 31 data are associated with False-Positives and 6 data are associated with True-Negatives**  
...
model_class_0.7 <- model_pred_df %>% 
                          mutate(pred_class = ifelse(model_pred_df$pred_prob > 0.7, 
                                                    "event", 
                                                    "non_event"))
...
model_class_0.3 <- model_pred_df %>% 
                          mutate(pred_class = ifelse(model_pred_df$pred_prob > 0.3, 
                                                    "event", 
                                                    "non_event"))
...
```{r, solution_03b}
### add more code chunks if you need to
Sensitivity_0.7 <- 26 / (26 + 40)
Sensitivity_0.7

Specificity_0.7 <- 58 / (1 + 58)
Specificity_0.7
```
...
```{r, solution_03c}
### your code here
Accuracy_0.3 <- (48 + 41) / (48 + 18 + 18 + 41)
Accuracy_0.3
```
...
```{r, solution_03d}
### add more code chunks if you need to
Sensitivity_0.3 <- 48 / (48 + 18)
Sensitivity_0.3

Specificity_0.3 <- 41 / (18 + 41)
Specificity_0.3
...
Sensitivity_all = c(Sensitivity_0.3,Sensitivity,Sensitivity_0.7)
FPR_all = c(FPR_0.3,FPR,FPR_0.7)

df <- data.frame(FPR_all,Sensitivity_all)

df %>% ggplot() +
       geom_line(mapping = aes(x = FPR_all, y = Sensitivity_all)) +
       geom_point(mapping = aes(x = FPR_all, y = Sensitivity_all)) +
...
**Because the mean of y > 0.4, we can consider it is balanced**
...
### model 1
set.seed(2021)
mod_1_acc <- train(y~x1+x2+x3+x4, data = df, method = "glm", metric = metric_acc, trControl = ctrl_acc)
mod_1_acc
```
...
**The accuracy of model 3 is the highest, so model 3 is considered the best**
...
**Best Model**
model 2: FP =  9.2 ;FN = 12.5
model 3: FP =  7.6 ;FN = 12.5
**Worst Model**
model 5: FP =  21.2 ;FN = 19.4
model 8: FP =  17.2 ;FN = 27.6
**Worsr Models' FP and FN larger than Best Models' FP and FN**
...
### model 1
set.seed(2021)
mod_1_roc <- train(y~x1+x2+x3+x4, data = df, method = "glm", metric = metric_roc, trControl = ctrl_roc)
...
**The result is still best for model 3**
...
**As the threshold decreases, more cases will be determined as events by the model, which will lead to an increase in TP and a decrease in FN. In addition, the Sensitivity will also become larger due to the change in TP and FN.**
...
```{r, solution_05g}
### add your code here
mod_1_roc$pred %>% group_by(Resample) %>%
                   roc_curve(obs, event) %>%
                   autoplot()
```
...
**According to the concept that we want the FPR to rapidly decrease as we increase the threshold, Model 3 and model 2 still are best models. In addition, Model 8 almost close to the 45 degree line, which means it is bad model.**
...
**For every data, the probability of event and non_event together equals 1. Every probility. Each number represents the probability of their occurrence in the same event. For event 1, even if it happens 10000 times, the probability of an 'event' is still 26%**
...
```{r, solution_06c}
### your code here
test_df_1_b <- test_df_1 %>% mutate(pred_bin = cut(event,
...
What do you think?  
**The (0.9,1] only have event but no pred_bin just have non_event**
...
**Almost identical, but with some errors and differences. For example, in (0.8, 0.9] there are some differences in the predicted probability of the previous models.**
...
```{r}
data_event_count <- test_df_1_b %>% group_by(pred_bin) %>% 
                                    filter(y == "event") %>% 
...
```{r}
data_count_all <- test_df_1_b %>% group_by(pred_bin) %>% 
                                  count(name = "all_count")
data_count_all
```
```{r}
my_calcurve_1 <- inner_join(data_event_count, data_count_all, by = "pred_bin")
my_calcurve_1
```
```{r}
my_calcurve_1 <- my_calcurve_1 %>% 
                 mutate(prop_event = event_count / all_count,
                        prop_non_event = 1 - (event_count / all_count))
my_calcurve_1
```

```{r}
temp <- seq(0.05,1,by = 0.1)

my_calcurve_1 <- my_calcurve_1 %>% add_column(mid_bin = temp)
my_calcurve_1
...
test_df_3_b <- test_df_3 %>% mutate(pred_bin = cut(event,
                             breaks = seq(0,1, by = 0.1),
                             include.lowest = TRUE))
test_df_3_b

test_df_8_b <-test_df_8 %>% mutate(pred_bin = cut(event,
                            breaks = seq(0,1, by = 0.1),
                            include.lowest = TRUE))
test_df_8_b
```
...
Add as many code chunks as you feel are necessary.  
```{r}
data_event_count_3 <- test_df_3_b %>% group_by(pred_bin) %>% 
                                    filter(y == "event") %>% 
                                    count(name = "event_count") 

data_count_all_3 <- test_df_3_b %>% group_by(pred_bin) %>% 
                                  count(name = "all_count")

my_calcurve_3 <- inner_join(data_count_all_3,data_event_count_3,by = "pred_bin")

my_calcurve_3 <- my_calcurve_3 %>% 
                 mutate(prop_event = event_count / all_count,
                        prop_non_event = 1 - (event_count / all_count))

my_calcurve_3 <- my_calcurve_3 %>% add_column(mid_bin = temp)
...
Add as many code chunks as you feel are necessary.
```{r}
data_event_count_8 <- test_df_8_b %>% group_by(pred_bin) %>% 
                                     filter(y == "event") %>% 
                                     count(name = "event_count") 

data_count_all_8 <- test_df_8_b %>% group_by(pred_bin) %>% 
                                  count(name = "all_count")

my_calcurve_8 <- inner_join(data_count_all_8,data_event_count_8,by = "pred_bin")
...
temp_8 <- c(0.35,0.45,0.55,0.65)

my_calcurve_8 <- my_calcurve_8 %>% add_column(mid_bin = temp_8)
...
**According to the calibration curves, it is model 3 that looks best. Although all three models have a point very far from the 45 degree line, model 3 is closer to the diagonal than models 1 and 8. Therefore, model 3 is considered to be better.**
