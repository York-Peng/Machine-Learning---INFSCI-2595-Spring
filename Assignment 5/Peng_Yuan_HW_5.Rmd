---
title: "INFSCI 2595 Spring 2023 Homework: 05"
subtitle: "Assigned February 9, 2023; Due: February 16, 2023"
author: "Peng Yuan"
date: "Submission time: February 16, 2023 at 11:00PM EST"
...
$$
\mu_{0} = \left( 135 + 115\right) / 2 = 125
\\ \tau_{0} = \left( 135 - 115  \right) / 4 = 5
$$
...
```{r}
quantiles_third <- qnorm(0.75, mean = 125, sd = 5)
quantiles_first <- qnorm(0.25, mean = 125, sd = 5)

quantiles_third
quantiles_first
```
```{r}
interval <- quantiles_third - quantiles_first
interval
```
**In the stand normal distribution, 0.6744 represents about 50% of the probability.**
...
```{r}
Probability_density <- pnorm(117, mean = 125, sd = 5)
Probability_density
```
...
\\  \frac{1}{\tau_{N}^{2}} = \frac{{\sigma^{2}} + N\tau_{0}^{2}}{\tau_{0}^{2} + {\sigma^{2}}}
\\ \frac{{\sigma^{2}} + \tau_{0}^{2}}{\tau_{N}^{2}} = {\sigma^{2}} + N\tau_{0}^{2}
\\ N = \frac{{\sigma^{2}} + \tau_{0}^{2} - \tau_{N}^{2}{\sigma^{2}}}{\tau_{N}^{2}\tau_0^{2}}
\\ N = \frac{{\sigma^{2}} \left( \tau_{0}^{2} - \tau_{N}^{2}\right)}{\tau_{N}^{2}\tau_0^{2}}
$$
```{r}
PSD_2 <- (5^2 *(5^2 - 2^2)) / (2^2 * 5^2)
PSD_2
PSD_0.5 <-   (5^2 *(5^2 - 0.5^2)) / (0.5^2 * 5^2)
PSD_0.5
```
...
\\ \frac{1}{\tau_{N}^{2}} = \frac{1}{\tau_{0}^{2}} + \frac{N}{\sigma^{2}} \rightarrow N = \frac{\sigma^{2}}{\tau_{N}^{2}}
$$
```{r}
num_obs <- 15^2 / 3^2
num_obs
...
Because
$$
\log \left[ normal \left( x_n \mid \mu, \sigma \right) \right] \rightarrow \log \left[ \frac{1}{\sqrt{2 \pi \sigma^{2}}} \exp \left(- \frac{1}{2 \sigma^{2}} \left(x_{n} - \mu \right)^{2} \right)\right]
$$
Thus
$$
\\ \log \left[p\left(\mu \mid \mathbf{x}, \sigma\right) \right] \propto \log \left[\prod_{n=1}^{N} \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right) \right] + \log \left[\mathrm{normal}\left( \mu \mid \mu_0, \tau_0\right) \right]
\\ \log \left[p\left(\mu \mid \mathbf{x}, \sigma\right) \right] \propto \log \left[\prod_{n=1}^{N} \left( \frac{1}{\sqrt{2 \pi \sigma^{2}}} \exp \left(- \frac{1}{2 \sigma^{2}} \left(x_{n} - \mu \right)^{2} \right) \right) \right] + \log \left[\frac{1}{\sqrt{2 \pi \tau_{0}^{2}}} \exp \left(- \frac{1}{2 \tau_{0}^{2}} \left(\mu - \mu_{0} \right)^{2} \right) \right]
\\ \log \left[p\left(\mu \mid \mathbf{x}, \sigma\right) \right] \propto - \frac{1}{2 \sigma^{2}} \sum_{n=1}^{N} \left[\left(x_{n} - \mu \right)^2 \right] - \frac{1}{2 \tau_{0}^{2}} \left(\mu - \mu_{0} \right)^2
$$
...
\\ \frac{\partial}{\partial \mu} \left(\log \left[p\left(\mu \mid \mathbf{x}, \sigma\right) \right] \right) \propto \frac{1}{\sigma^{2}} \sum_{n=1}^{N} \left( x_{n} - \mu \right) - \frac{1}{\tau_{0}^{2}} \mu - \mu_{0}
...
You may add as many equation blocks as you see fit to answer this question.  
$$
N \left( \frac{\bar{x} - \mu}{\sigma^{2}} \right) -\frac{\mu - \mu_{0}}{\tau_{0}^{2}} = 0
$$
$$
\frac{\bar{x} - \mu}{\sigma^{2}} = \frac{\mu - \mu_{0}}{\tau_{0}^{2}N}
$$

$$
\tau_{0}^{2}N\bar{x} + \sigma^{2}\mu_0 = \sigma^{2}\mu + \tau_{0}^{2}N\mu
$$
...
$$
\mu_{MAP} = \frac{\tau_{0}^{2}N\bar{x} + \sigma^{2}\mu_0}{\sigma^{2} + \tau_{0}^{2}N}
$$
$$
\mu_{MAP} = \frac{\frac{1}{\tau_{0}^{2}} \mu_{0} + \frac{N}{\sigma^{2}} \bar{x}}{\frac{1}{\tau_{0}^{2}} + \frac{N}{\sigma^{2}}}
$$
...
**The same expression as for Posterior Mean. The images of Gaussian distribution or standard distribution are both symmetric. The calculation result represents their maximum value, so both are the same.**
...
$$
\log \left[p \left(\sigma \mid \mathbf{x}, \mu\right) \right] \propto \log \left[\prod_{n=1}^{N} \left( \frac{1}{\sqrt{2 \pi \sigma^{2}}} \exp \left(- \frac{1}{2 \sigma^{2}} \left(x_{n} - \mu \right)^{2} \right) \right) \right] + \log \left[ \lambda \exp \left(-\lambda \sigma \right)\right]
$$
$$
\log \left[p \left(\sigma \mid \mathbf{x}, \mu\right) \right] \propto N \log \left(\frac{1}{\sigma} \right) - \frac{1}{2 \sigma^{2}} \sum_{n=1}^{N} \left[\left(x_{n} - \mu \right)^2 \right] - \lambda \sigma
$$
...
Add as many equation blocks as you see fit to answer this question.  
$$
vN =  \sum_{n=1}^{N}\left( \left(x_n - \mu\right)^2 \right)
\\ \log \left[p \left(\sigma \mid \mathbf{x}, \mu\right) \right] \propto N \log \left(\frac{1}{\sigma} \right) - \frac{Nv}{2 \sigma^{2}} - \lambda \sigma
$$
...
What do you think?  
**There is only one mode and the image doesn't look like a parabola**
...
Add as many equation blocks as you see fit to answer this question.  
$$
\sigma = \exp \left(\varphi \right)
\\ \frac{\partial}{\partial \varphi} \sigma = \frac{\partial}{\partial \varphi} \exp \left(\varphi \right)
\\ \frac{\partial}{\partial \varphi} \sigma = \exp \left(\varphi \right)
\\ \log \left(\frac{\partial}{\partial \varphi} \sigma \right) = \log \left(\exp \left(\varphi \right) \right)
\\ \log \left(\frac{\partial}{\partial \varphi} \sigma \right) = \varphi
$$
...
log_post_varphi_unnorm <- function(varphi, N, v, lambda)
{
  ...
  log_derivative <- log_post_sigma_unnorm(sigma, N, v, lambda) + varphi
...
```
What do you think?  
**Compared to the image in 3d, this image is more like a parabola**
...
opt_sine_01
```
**When we need to maximize, we need to set the fnscale in the list to a negative number. Without passing in a gradient function, you can set gr to NULL**
...
{
  result <- -exp((x - constants$a) ^ constants$m)
  return(result)
}
...
tibble::tibble(
  ...
  ggplot(mapping = aes(x = x, 
                       y = my_func(x,my_constants)))+
  geom_line() +
  geom_vline(xintercept = opt_result$par, color = "red",linetype = "dashed")
```
...
tibble::tibble(
  ...
  mutate(log_result = log_post_varphi_unnorm(varphi, N, v, 0.75)) %>% 
         ggplot(mapping = aes(x = varphi, y = log_result)) +
         geom_line() +
         geom_vline(xintercept = varphi_opt_b$par, color = "red",linetype = "dashed")
```
