...
fit_lm_1 <- lm(y ~ x, as.data.frame(df_low_high))
...
fit_lm_2 <- lm(y ~ x + I(x^2), as.data.frame(df_low_high))
...
fit_lm_3 <- lm(y ~ x + I(x^2) + I(x^3), as.data.frame(df_low_high))
...
### degree 4
fit_lm_4 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4), as.data.frame(df_low_high))
...
### degree 5
fit_lm_5 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5), as.data.frame(df_low_high))
...
### degree 6
fit_lm_6 <-lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6), as.data.frame(df_low_high))
...
### degree 7
fit_lm_7 <-lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7), as.data.frame(df_low_high))
...
### degree 8
fit_lm_8 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), as.data.frame(df_low_high))
...
### degree 9
fit_lm_9 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8) + I(x^9), as.data.frame(df_low_high))
...
**The result of R-squared shows that fit_lm_9 is the greatest and it's RMSE value is the lowest,so fit_lm_9 is considered to be the best model. **
...
**<1> For uncertainty, the inaccuracy of model 9 is the highest, because his line is the longest, which means that he may have more uncertainty values. Model 2 is the most accurate**

**<2> The models 6 and 9 lack coefficient because their lines intersect with 0, which indicates that it is difficult to judge their results. And the results of model 2 are more clear.**
...
### degree 5
viz_trend_5 <- predict(fit_lm_4,input_viz)
...
df_viz_1 %>%
  ggplot(mapping = aes(x=x)) +
  geom_point(mapping = aes(y = y), shape = 0, color = 'red',data = df_low_high) +
...
**Looking through the images of 2j, model 2 is the best. The trend of the prediction model 2 is consistent with the trend of the data and there is no over-fitting of the data.**

**Although the prediction models 7, 8, and 9 appear to overlap with the data the most, they overlap too much with the monitoring data and suffer from over-fitting problems. This resulted in several models with poor generalization capabilities that are not applicable to predict other data.**
...
df_low_high %>%
  ggplot(mapping = aes(x=x)) +
  geom_point(mapping = aes(y=y), shape = 2,color = 'red', data = df_train) +
  geom_point(mapping = aes(y=y), shape = 1,color = 'blue', data = df_holdout)
```
...
**The RMSE calculated from the hold-out evaluation shows that model 2 is the best. This is inconsistent with the results obtained from the previous training set, where the ninth model was the best. Here the dataset is randomly sampled and fitted with hold-out data after the prediction model is obtained using the training set.**
...
**For the coefficients, neither model is significant since both models have more intersections of the data lines with 0.**

**The comparison of the two models reveals that the model uncertainty is greater for fit_train. This is due to the fact that fit_train contains more data compared to the fit_lm dataset, and the already high uncertainty of the model will show a larger confidence interval.**
...
**The coefficients of model 2 are more meaningful except that the same uncertainty variation pattern appears as in model 9. Most of whole lines of model 2 are in the negative or positive interval, with fewer cross sections.**
...
my_ctrl <- trainControl(method = "cv",number = 5, savePredictions = TRUE)
...
**A total of 5 times will be performed **
...
**The response in this experiment is continuous, so we can only choose from "RMSE", "Rsquared", and "MAE" methods. is more responsive to the error of the data than MAE and Rsquared, and better reflects the discrete nature and accuracy of the model.**
...
**The fit_3 model is the best.Because it's RMSE value is the lowest.**
...
**The coefficients of the models are similar for those in the intercept as well as for y=x+x^2. The uncertainty of mod_lowhigh_3 is the largest and that of mod_lowhigh_2 is the smallest. In addition, the uncertainty of the model varies less.**
...
```
**Compared with the image of 1a, the image of low level of noise has a more linear pattern.**
...
summary(lowlow_results,metric = "RMSE")
```
...
**Model 6 has the lowest RMSE value, so model 6 is the best**  
...
**The results of low level of noise and high level of noise are not similar. By calculating the RMSE, the final optimal models are not the same. However, the worst models did not perform well in either case. model 4, 3 and 2 performed more consistently, and model 6 performed significantly better after modifying the noise level.**
...
df_high_high %>%
  ...
  geom_point(mapping = aes(x=x,y=y))
```
**Compared with the 1a image, the high sample size image also has more obvious linearity**
...
**Model 5 has the lowest RMSE, so model 5 is the best**
...
**The performance of model 1, model 2 and model 3 was found to be much worse when the sample size was increased again. The models with average performance were optimized after increasing the data size. However, the worst model was still model 9.**
...
**The larger the sample size, it is easier to identify complex behavior. This allows the model to be optimized when it was not performing well in the low sample size case.**

**The low noise level reduces the prediction error of the model and makes the RMSE calculation more accurate. However, it is difficult to determine the performance when using a model that performs well in the low noise level case to fit real data with high uncertainty.**
