...
**It is difficult to see the trend between the inputs and response in both graphs. x2, however, seems to have a slight pattern.**
...
coefplot(mod01)
```
**The image of x1 as a whole is to the right of 0, which means that x1 could be statistically significant. While x2 intersects 0, it could not be statistically significant**
...
**According to the result, those three features all could be statistically significant, because they are all to the left or right of 0**
...
coefplot(mod03)
```
**Only x2^2 feature could be statistically significant.**
...
coefplot(mod04)
```
**All features could not be statistical significance**
...
**Fit a model which interacts a 12 degree-of-freedom natural spline from `x1` with linear, quadrtic, cubic, and quartic (4th degree) polyonomial features from `x2`. Assign the result to `mod06`.**  
...
```{r, solution_01h}
### add more code chunks if you like
broom::glance(mod01)$r.squared
broom::glance(mod01)$AIC
broom::glance(mod01)$BIC
```

```{r}
### add more code chunks if you like
broom::glance(mod02)$r.squared
broom::glance(mod02)$AIC
broom::glance(mod02)$BIC
```

```{r}
### add more code chunks if you like
broom::glance(mod03)$r.squared
broom::glance(mod03)$AIC
broom::glance(mod03)$BIC
```

```{r}
### add more code chunks if you like
broom::glance(mod04)$r.squared
broom::glance(mod04)$AIC
broom::glance(mod04)$BIC
```

```{r}
### add more code chunks if you like
broom::glance(mod05)$r.squared
broom::glance(mod05)$AIC
broom::glance(mod05)$BIC
```

```{r}
### add more code chunks if you like
broom::glance(mod06)$r.squared
broom::glance(mod06)$AIC
broom::glance(mod06)$BIC
```

**In terms of the R.square, the value of model-6 is the largest, which means that model-6 is the best. The BIC of model-6 is the largest and the value of AIC is not small, which means the model complexity of model-6 is very high. We need to choose a relatively simple model, so I think model-6 is not the best model.**

**In terms of the BIC and AIC, model-3 is the smallest. In addition, the r.squared of the model-3 was maintained at an acceptable level. Therefore I think model-3 is the best model in this.**
...
**Yes. As the AIC/BIC increases, the image exhibits an increase in the confidence interval but a decrease in the prediction interval. Although this indicates that our model has more possibilities to cover more values into the confidence interval, it also leads to an unstable prediction curve. We found that the curve started to fluctuate dramatically at the beginning of model4. This indicates that our model has difficulty reaching a trend, which is a sign of model deterioration.**
...
post_03_means <- laplace_03_weak$mode[1:ncol(X03)]
post_03_sd <- sqrt(diag(laplace_03_weak$var_matrix)[1:ncol(X03)])
viz_post_coefs(post_03_means, post_03_sd, colnames(X03))
```


```{r, solution_03d_b}
### make the posterior coefficient visualization for model 6
post_06_means <- laplace_06_weak$mode[1:ncol(X06)]
post_06_sd <- sqrt(diag(laplace_06_weak$var_matrix)[1:ncol(X06)])
viz_post_coefs(post_06_means, post_06_sd, colnames(X06))
```
...
**Because the Bayes_Factor larger than 1, we can consider model-3 better than model-6**
...
```{r, solution_03g}
### add more code chunks if you like
post_03_means_strong <- laplace_03_strong$mode[1:ncol(X03)]
post_03_sd_strong <- sqrt(diag(laplace_03_strong$var_matrix)[1:ncol(X03)])
viz_post_coefs(post_03_means_strong, post_03_sd_strong, colnames(X03))
```

```{r}
post_06_means_strong <- laplace_06_strong$mode[1:ncol(X06)]
post_06_sd_strong <- sqrt(diag(laplace_06_strong$var_matrix)[1:ncol(X06)])
viz_post_coefs(post_06_means_strong, post_06_sd_strong, colnames(X06))
```
...
```{r, solution_03h_b}
### add more code chunks if you like
laplace_03_very_strong <- my_laplace(rep(0,ncol(X03) + 1), 
                                     lm_logpost,info_03_very_strong)
laplace_03_very_strong$converge
laplace_06_very_strong <- my_laplace(rep(0,ncol(X06) + 1), 
                                     lm_logpost,info_06_very_strong)
...
```{r, solution_03i}
### add more code chunks if you like
post_03_means_very_strong <- laplace_03_very_strong$mode[1:ncol(X03)]
post_03_sd_very_strong <- sqrt(diag(laplace_03_very_strong$var_matrix)[1:ncol(X03)])
viz_post_coefs(post_03_means_very_strong, post_03_sd_very_strong, colnames(X03))
```

```{r, solution_03i_b}
### add more code chunks if you like
post_06_means_very_strong <- laplace_06_very_strong$mode[1:ncol(X06)]
post_06_sd_very_strong <- sqrt(diag(laplace_06_very_strong$var_matrix)[1:ncol(X06)])
viz_post_coefs(post_06_means_very_strong, post_06_sd_very_strong, colnames(X06))
```
...
**As our prior becomes more accurate, the greater the effect on the posterior. The standard deviation of the prior provides a constraint on the posterior so that the posterior must fall within the interval. As we keep adjusting the standard deviation higher (from 50 to 1 to 1/50), the coefficients shrink, which makes the model imprecise instead.**
...
```{r, solution_03k}
### add more code chunks if you like
Bayes_Factors_strong = exp(laplace_03_strong$log_evidence) / exp(laplace_06_strong$log_evidence)
Bayes_Factors_strong
```

```{r}
Bayes_Factors_very_strong = exp(laplace_03_very_strong$log_evidence) /exp(laplace_06_very_strong$log_evidence)
Bayes_Factors_very_strong
```

**Combined with our previous experimental results of BIC and AIC, we can find that model6 is less flexible relative to model3 and appears to be overfitted. When we adjust the standard deviation to 1/50, Bayes_Factors is less than 1, which means that model6 is better than model3 at this time. When the standard deviation is 1/50, it means that we have a very strong restriction on the model, and model6 is better at this time in accordance with our previous conclusion. Again, this means that we should not restrict the standard deviation too small, which would make the model less flexible and more complex.**
...
post_pred_summary_viz_03_very_strong <-
  summarize_lm_pred_from_laplace(laplace_03_very_strong, X03_test, 5000)
```
...
X06_test <- model.matrix( ~ splines::ns(x1, knots = knots$interior_knots) * (x2 + I(x2 ^2) + I(x2^3) + I(x2^4)), data = viz_grid)
...
**Yes. They are almost the same. The standard deviation of the better-than-priori is large and the output is restricted to a very small area centered on the mean of the prior. So we get results in which the interval is shrinking.**
...
```
**The dashed line in the figure marks the model that most closely resembles the minimum error. Since we should choose the simplest model, there is only one feature for both model-3 and model-6.**
...
**No. Two final tuned models are same. Both features of them are I(x2^2) **
