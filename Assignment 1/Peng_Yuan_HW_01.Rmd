---
title: "INFSCI 2595 Spring 2023 Homework: 01"
subtitle: "Assigned January 11, 2023; Due: January 18, 2023"
author: "PENG YUAN"
date: "Submission time: January 18, 2023 at 11:00PM EST"
...
iris %>%
  ggplot(mapping = aes(x = Sepal.Length))+
  geom_histogram(bins = 6,color = "navyblue", fill = "gold", alpha = 0.5)
```
...
**<1> The median represents the average of the samples, so the J color sample has the highest average price and the E color has the lowest. The overall trend shows that the more to the right the color sample is, the higher the value.**

**<2> The width of the box(which means the interquartile range) reflects the degree of fluctuation of the data. Therefore, it can be seen that the data of groups G, H, I, and J fluctuate more, while the data of groups D, E, and F fluctuate less **

**<3> The bolded lines represent the outliers, with E having the most outliers and J having the least outliers. The overall trend is that there are fewer outliers as you move to the right.**
...
diamonds %>%
  ggplot() + 
  facet_grid(clarity ~ cut)+
  geom_boxplot(mapping = aes(x=color, y=price, fill = carat>median(carat)))+
...
filter(cut=="Ideal", clarity=="IF") %>%
  ...
  geom_boxplot(mapping = aes(x=color, y=price, fill = carat>median(diamonds$carat)))+
  theme(legend.position = "top")
```
...
**<1> Carat is the factor that brings the most influence to price. The addition of this factor has increased the median price of all colors of diamonds by a significant amount. It also influences the overall median trend.**

**<2> In addition, the addition of carat has greatly influenced the fluctuation of the value of different color diamonds. the range of J color value fluctuation has increased again. However, the overall trend in the size of the range has not changed. **

**<3> The difference between the price of diamonds above and below the median for the same color is significant.**

**<4> The overall price of D, E, F, and G color diamonds is high and stable, and the D color diamonds are particularly strong. J color diamonds also have high prices, but the value fluctuates too much.**
...
**lm(formula, dataframe, subset, weights, na.action, method = "qr", model = TRUE, x = FALSE, y = FALSE, qr = TRUE, singular.ok = TRUE, contrasts = NULL, offset, ...)**
...
$$ 
\left( \mathbf{X}_{1} \right)^{\mathbf{Z}^\mathbf{Y}} 
$$
