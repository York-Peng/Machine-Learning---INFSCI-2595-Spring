...
```{r}
quad_data_big <- rowid_to_column(quad_data_big, "rowid")
```

```{r}
quad_data_big %>% ggplot(aes(x = x, y = y)) +
                  geom_point(aes(color = rowid <= 5), size = 3) +
...
```{r}
print(Xmat_quad_small)
```

**Both Xmat_quad_small and Xmat_quad_big have 3 columns**

**In the Xmat_quad_small's first column, all values are 1. It was originally intercept or became bias in the formula. We create a "fake" variable in matrix, which is defined to always equal 1**
...
**Both big and small posterior covariance matrix have 3 columns**
...
```{r}
posterior_sd_quad_small <- sqrt(diag(covmat_quad_small_noprior))
posterior_sd_quad_big <- sqrt(diag(covmat_quad_big_noprior))
posterior_sd_quad_small
posterior_sd_quad_big
```

**Small data case's standard deviation is 0.9713975 and Big data case's standard deviation is 0.2200419**
...
**Yes, if we change the $\sigma$, posterior means will be changed. Because the complete formula should be "$\sigma^2$ solve(t(Xmat_quad_big) %*% Xmat_quad_big) %*% t(Xmat_quad_big) %*% y_quad_big_col". Thus, the value of sigma will affect posterior means**
...
```{r}
print(sqrt(covmat_quad_small_noprior[3,3]) + c(-2,2) * 1)
print(sqrt(covmat_quad_big_noprior[3,3]) + c(-2,2) * 1)
```

What do you think?  
**Both the small and big data case have statistically significant**
...
covmat_quad_small_inform <- cov2cor(solve(solve(B0_inform) + (t(Xmat_quad_small) %*% Xmat_quad_small)/(1^2)))
```


```{r, solution_for_2b_b, eval=TRUE}
### posterior covariance matrix INFORMATIVE prior and BIG data
covmat_quad_big_inform <- cov2cor(solve(solve(B0_inform) + (t(Xmat_quad_big) %*% Xmat_quad_big)/(1^2)))
```
...
covmat_quad_small_vague <- cov2cor(solve(solve(B0_vague) + (t(Xmat_quad_small) %*% Xmat_quad_small)/(1^2)))
```


```{r, solution_for_2c_b, eval=TRUE}
### posterior covariance matrix VAGUE prior and BIG data
covmat_quad_big_vague <- cov2cor(solve(solve(B0_vague) + (t(Xmat_quad_small) %*% Xmat_quad_small)/(1^2)))
```
...
```{r, 2_d_small}
posterior_sd_quad_small_INFORMATIVE <- sqrt(diag((solve(solve(B0_inform) + (t(Xmat_quad_small) %*% Xmat_quad_small)/(1^2)))))
posterior_sd_quad_small_INFORMATIVE
posterior_sd_quad_small
```

```{r, 2_d_big}
posterior_sd_quad_big_INFORMATIVE <- sqrt(diag(solve(solve(B0_inform) + (t(Xmat_quad_big) %*% Xmat_quad_big)/(1^2))))
posterior_sd_quad_big_INFORMATIVE
posterior_sd_quad_big
```

**In small case, there is a significant reduction in the posterior standard deviation. In the big case, the posterior standard deviation has a less pronounced reduction **
...
Insert your code chunks here.  
```{r, 2_e_small}
posterior_sd_quad_small_VAGUE <- sqrt(diag(solve(solve(B0_vague) + (t(Xmat_quad_small) %*% Xmat_quad_small)/(1^2))))
posterior_sd_quad_small_VAGUE
posterior_sd_quad_small
```

```{r, 2_e_big}
posterior_sd_quad_big_VAGUE <- sqrt(diag(solve(solve(B0_vague) + (t(Xmat_quad_big) %*% Xmat_quad_big)/(1^2))))
posterior_sd_quad_big_VAGUE
posterior_sd_quad_big
```

**There is no significant change in both sets of data, and the calculated results are similar to 1f.**
...
**According to the standard deviation, prior nicely bounded the linear model regression posterior results. It will make the Posterior precision become higher.**
...
Xmat_big <- model.matrix(y~x + I(x^2), data = quad_data_big)

info_quad <- list(
  yobs = quad_data_big$y,
  design_matrix = Xmat_big,
  mu_beta = 0,
  tau_beta = 3,
  sigma_rate = 1
)
...
```{r, solution_03b_c}
###
parameters <- rep(-1, ncol(info_quad$design_matrix)+1)

lm_logpost(parameters, info_quad)
```
...
start_guess <- rep(0, ncol(info_quad$design_matrix)+1)
laplace_quad <- my_laplace(start_guess, lm_logpost, info_quad)
```
...
**No, we don’t need to worry about the affect of initial guess. Because there is only one mode in the approximation of linear model**
...
post_trend_samples_quad <- make_post_lm_pred(Xmat_big, post_samples_quad)
```
...
**The length of error_quad_post_01 is 30. **

```{r, solution_04d}
###
print(length(error_quad_post_01))
```
...
**Their MAE values are not same. Because they use different beta value.**
...
We should use colMeans() function. Because each column represents a posterior sample. 

```{r, solution_04f, eval=TRUE}
MAE_quad <- colMeans(absError_quad_mat)

### data type?
print(class(MAE_quad))
### length?
print(length(MAE_quad))
```
...
**Because MAE is influenced by beta, but Bayes has no point estimate for beta. Therefore MAE is uncertain.**
...
```{r solution_05a}
###
data <- data.frame(train_set = train_df, test_set = test_df)
...
make_spline_basis_mats <- function(J, train_data, test_data)
{
  ...
  train_matrix <- model.matrix( ~ splines::ns(x = x, knots = interior_knots,Boundary.knots = boundary_knots), data = train_data)
  
  test_matrix <- model.matrix( ~ splines::ns(x = x, knots = interior_knots, Boundary.knots = boundary_knots), data = test_data)
...
**We can see in the boxplots that the MAE of the test set is significantly larger than the train set starting from the eighth model position.**
**In the line graph, also starting from the eighth model position, the MAE trend of the test data set is significantly different from the MAE trend of the train data set.**
...
**According to the result, the 7th model is the best and 9th model is also good.**
...
**Their evaluation results are different. In the plots of 5e and 5f we can find that the MAE of both the test and training sets are decreasing until model 8, but there is no big change afterwards. In this problem, we think that model 7 is the best, but it is not definite. In the Evidence-based assessment, as the degree increases, the model becomes more heterogeneous and complex, and the more likely to have high consumption and instability.**
