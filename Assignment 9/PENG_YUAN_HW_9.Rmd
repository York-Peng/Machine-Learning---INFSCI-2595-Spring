...
dfC %>% ggplot(aes(x = x, y = y, color = v)) +
        geom_point(size = 4)
```
Just looking at the scatter plot, only a weaker relationship can be seen
...
```
Yes, with the addition of smooth's straight lines, the relationship becomes easier to observe. They have different levels of v and slope. 
...
According to the AIC and the R-squared, modC_int is better. 
This result is consistent with the 6d. The result of modC_int proves the interaction of v with x.
