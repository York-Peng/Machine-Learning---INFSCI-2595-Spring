---
title: "INFSCI 2595 Spring 2023 Homework: 06"
subtitle: "Assigned February 16, 2023; Due: February 23, 2023"
author: "Peng Yuan"
date: "Submission time: February 23, 2023 at 11:00PM EST"
...
min_value <- min(hw06_df$x)
min_value

max_value <-max(hw06_df$x)
max_value
...
$$
P \left( 10 ≤ \mu ≤ 12 \right) = 0.95
\\ \mu_0 = \left( 10+12 \right) / 2 = 11
\\ \tau_0 = \frac{\left( 12 - 11\right )}{1.96} = 0.5
$$
...
eval_logpost <- function(unknown_1, unknown_2, logpost_func, logpost_info)
{
  ...
  eval_log_result <- logpost_func(unknown,logpost_info)
  return(eval_log_result)
}
```
...
The code chunk below uses the `purrr::map2_dfr()` function to apply the `eval_logpost()` function to all combinations of `mu` and `sigma` within `param_grid`. Be sure to set the `eval` flag to `TRUE` after you run the code chunk, because by default `eval=TRUE`. The result is assigned to the variable `log_post_result`. You can check the RStudio Environment Panel to see that the length of `log_post_result` is equal to the number of rows in `param_grid`.  
...
```
What do you think?  
**Most of the samples are in the interval and only a small fraction does not belong to the posterior model. The vertical length of the surface represents the uncertainty of sigma and the horizontal width of the surface represents the uncertainty of mu. Thus, we know that sigma is between 10 and 1 and mu is between 9.5 and 13.5.In addition, the gray parts indicate unlikely values.**
...
$$
\frac{\partial}{\partial \mu} \left(\log \left[p\left(\mu \mid \mathbf{x}, \sigma\right) \right] \right) \propto  \sum_{n=1}^{N} \left[- \frac{1}{2 \sigma^{2}} \frac{\partial}{\partial \mu} \left(x_{n} - \mu \right)^{2} \right] - \frac{1}{2 \tau_{0}^{2}} \frac{\partial}{\partial \mu} \left(\mu - \mu_{0} \right)^{2}
$$
...
$$
\frac{\partial}{\partial \mu} \left(\log \left[p\left(\mu \mid \mathbf{x}, \sigma\right) \right] \right) \propto N \left( \frac{\bar{x} - \mu}{\sigma^{2}} \right) -\frac{\mu - \mu_{0}}{\tau_{0}^{2}} 
$$
$$
\frac{\partial}{\partial \mu} (\log [p(\mu \mid \mathbf{x}, \sigma)]) \propto \frac{N\bar{x}}{\sigma^{2}} - \frac{N\mu}{\sigma^{2}} - \frac{\mu}{\tau_{0}^{2}} - \frac{\mu_{0}}{\tau_{0}^{2}}
$$

$$
\frac{\partial}{\partial \mu} (\log [p(\mu \mid \mathbf{x}, \sigma)]) \propto - \frac{N}{\sigma^{2}} - \frac{1}{\tau_{0}^2}
$$
...
What do you think?  
**According to the equation we can see that the right side of the equation must be a negative number. Therefore the left side of the equation also needs to be negative, which means mu is negative. So we can assume that it is the maximum log-posterior density and not the minimum log-posterior density**
...
Because of the normal distribution, the approximate posterior mean is equivalent to posterior mode.
$$
...
**The Laplace approximation uses the second-order derivative to approximate the log-posterior distribution and it is a symmetric Gaussian distribution. This approximation provides an exact second-order moment for the posterior distribution. Thus when there is only one unknown parameter, the symmetric distribution can perfectly explain the probability density distribution. **
...
What do you think?  
**Under the condition that both the prior and the likelihood obey the Gaussian distribution, the boundary on sigma will be ignored since the Gaussian distribution is less concerned with the boundary. But we do not want it to be ignored. Therefore, we need to use the transformation to apply bounds to the results. After applying the transformation, we obtain a more accurate posterior distribution based on the transformed variable varphi.**
...
$$
p \left( \mu, \varphi \mid \mathbf{x} \right) = \prod_{n=1}^{N} \left( \mathrm{normal} \left(x_n \mid \mu, g^{-1} \left(\varphi \right) \right) \right) \times \mathrm{normal}\left(\mu \mid \mu_0, \tau_0\right) 
...
log_post_cv_result <- purrr::map2_dbl(cv_param_grid$mu, cv_param_grid$varphi, 
                                      eval_logpost, logpost_func = my_cv_logpost, 
                                      logpost_info = hw06_info)
## log_post_cv_result
```
...
cv_param_grid %>% 
  ...
  theme_bw()
```
**The overall contours of the image are more aggregated and appear more symmetrical. This is because in the transformation, we give varphi a boundary**
...
map_res_01 <- optim(init_guess_01, my_cv_logpost, gr = NULL, hw06_info, method = "BFGS", hessian = TRUE, 
...
map_res_02 <- optim(init_guess_02, my_cv_logpost, gr = NULL, hw06_info, method = "BFGS", hessian = TRUE, 
                    control = list(fnscale = -1, maxit = 1001))
map_res_02
```
...
laplace_result1 <-  my_laplace(init_guess_01, my_cv_logpost, hw06_info)
laplace_result2 <- my_laplace(init_guess_02, my_cv_logpost, hw06_info)
```

Include as many code chunks and discussion text as you feel are necessary.  
```{r}
laplace_result1[1]$mode
laplace_result2[1]$mode
```


```{r}
sqrt(laplace_result1$var_matrix[1])
sqrt(laplace_result2$var_matrix[1])
```


```{r}
sqrt(laplace_result1$var_matrix[4])
sqrt(laplace_result2$var_matrix[4])
```

```{r}
cor_coe_1 <- (laplace_result1$var_matrix[2] / (sqrt(laplace_result1$var_matrix[1])*sqrt(laplace_result1$var_matrix[4])))

cor_coe_2 <- (laplace_result2$var_matrix[2] / (sqrt(laplace_result2$var_matrix[1])*sqrt(laplace_result2$var_matrix[4])))
...
Include as many code chunks and discussion text as you feel are necessary.  
```{r}
pnorm(sample_average, mean = laplace_result1$mode[1], sd = sqrt(laplace_result1$var_matrix[1]))
```
...
set.seed(202004)
post_samples <-  generate_post_samples(laplace_result1, 1e4)
post_samples
```
...
**According to the diagram, the posterior distribution on $\sigma$ is not like Gaussian**
